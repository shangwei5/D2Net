...
import time
import argparse
from model.d2net_event import D2Net
...
class Inference:
    def __init__(self, args):
        ...
        # self.label_path = os.path.join(self.data_path, "label")  #"detect_label"
    ...
    def infer(self):
        with torch.no_grad():
            ...
            for v in videos:
                ...
                for in_seq, gt_seq, pre_seq, sub_seq, event_seq in zip(input_seqs, gt_seqs, PreSIndex_list, SubSIndex_list, event_seqs):   #, bm_seq, label_seq   , bm_seqs, label_seqs
                    ...
                    events = [txt2npy(p).transpose(1,2,0) for p in event_seq]
                    if v == 'books':
                        gt = imageio.imread(gt_seq[self.n_seq // 2+1])
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='../dataset/GOPRO_Random/test',
                        help='the path of test data')
    parser.add_argument('--model_path', type=str, default='./logs/model_best_event.pt',
    ...
    if args.default_data == 'GOPRO':
        ...
        args.model_path = './logs/model_best_event.pt'
        args.result_path = '../D2Net_results/GOPRO_event_results'
