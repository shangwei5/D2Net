...
class D2Net(nn.Module):
    ...
    def __init__(self, in_channels=3, n_sequence=3, out_channels=3, n_resblock=3, n_feat=32,
        ...
        self.recons_net_near = recons_video.RECONS_VIDEO(in_channels=in_channels, n_sequence=3, out_channels=out_channels,
                                                    ...
                                                    extra_channels=extra_channels, need_event=True)
    ...
    def forward(self, x, event, isTraing=False):
        frame_list = [x[:, i, :, :, :] for i in range(self.n_sequence)]
        event_list = [event[:, i, :, :, :] for i in range(self.n_sequence-2)]


        # Interation 1
        warped_l0, _, _, flow_mask_l0 = self.flow_net_nsf(frame_list[1], frame_list[0])
        warped_r0, _, _, flow_mask_r0 = self.flow_net_nsf(frame_list[1], frame_list[2])
        warped_lm, _, _, flow_mask_lm = self.flow_net_nsf(frame_list[self.n_sequence // 2], frame_list[1])
        warped_rm, _, _, flow_mask_rm = self.flow_net_nsf(frame_list[self.n_sequence // 2], frame_list[3])
        warped_l2, _, _, flow_mask_l2 = self.flow_net_nsf(frame_list[3], frame_list[2])
        warped_r2, _, _, flow_mask_r2 = self.flow_net_nsf(frame_list[3], frame_list[4])

        states_m = None
        concated = torch.cat([warped_l0, frame_list[1], warped_r0], dim=1)  #frame_list[0]
        recons_1, states_m, _, _ = self.recons_net_nsf(concated, states_m, event_list[0])

        concated = torch.cat([warped_lm, frame_list[self.n_sequence//2], warped_rm], dim=1)
        recons_2, states_m, _, _ = self.recons_net_nsf(concated, states_m, event_list[1])

        concated = torch.cat([warped_l2, frame_list[3], warped_r2], dim=1)   #frame_list[2]
        recons_3, states_m, _, _ = self.recons_net_nsf(concated, states_m, event_list[2])
        ...
        out, _, _, _ = self.recons_net_near(concated, None, event_list[1])
