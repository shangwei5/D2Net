...
import time
import argparse
from model.d2net_offical import D2Net
...
class Inference:
    ...
    def infer(self):
        with torch.no_grad():
            total_psnr = {}
            total_ssim = {}
            videos = sorted(os.listdir(self.data_path))    #self.input_path
            # videos = sorted(os.listdir(os.path.join(self.data_path, "blur")))
            # print(videos)
            for v in videos:
                video_psnr = []
                video_ssim = []
                input_frames = sorted(glob.glob(os.path.join(self.data_path, v, "blur", "*")))
                gt_frames = sorted(glob.glob(os.path.join(self.data_path, v, "sharp", "*")))
                # input_frames = sorted(glob.glob(os.path.join(self.data_path, "blur", v, "*")))
                # gt_frames = sorted(glob.glob(os.path.join(self.data_path, "sharp", v, "*")))
    ...
    def forward_chop(self, *args, shave_h=20, shave_w=20, min_size=160000):
        ...
        if h * w < 6 * min_size:
            for i in range(0, 4, n_GPUs):
                ...
                # print(x[0].size())
                # print(x[1].size())
                y =  P.data_parallel(self.net.module, *x, range(n_GPUs))  #*x
...
if __name__ == '__main__':
    ...
    if args.default_data == 'GOPRO':
        args.model_path = './offical_logs/model_best.pt'
        args.result_path = '../D2Net_offical_results/D2Net_offical'
        args.n_sequence = 5
        args.n_GPUs = 2
